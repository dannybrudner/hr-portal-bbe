  ...
  try {
    for (const manager of managers) {
      await resend.emails.send({
        ...
        to: manager.email,
        subject: `New Leave Request: ${escapeHtml(String(employeeName ?? ""))} — ${escapeHtml(String(leaveType ?? ""))} (${days} days)`,
          ...
          <div style="padding:32px;">
            ...
            <div style="background:#2e2e2e;border-radius:12px;padding:20px;margin-bottom:24px;border-left:3px solid #C9A227;">
              <table style="width:100%;border-collapse:collapse;">
                <tr><td style="padding:6px 0;font-size:12px;color:#9aa0b4;width:40%;">Employee</td><td style="font-weight:600;">${escapeHtml(String(employeeName ?? ""))}</td></tr>
                <tr><td style="padding:6px 0;font-size:12px;color:#9aa0b4;">Email</td><td style="font-size:13px;">${escapeHtml(String(employeeEmail ?? ""))}</td></tr>
                <tr><td style="padding:6px 0;font-size:12px;color:#9aa0b4;">Type</td><td style="font-weight:600;color:#C9A227;">${escapeHtml(String(leaveType ?? ""))}</td></tr>
                <tr><td style="padding:6px 0;font-size:12px;color:#9aa0b4;">Dates</td><td style="font-weight:600;">${escapeHtml(String(startDate ?? ""))} → ${escapeHtml(String(endDate ?? ""))} (${days} day${days>1?'s':''})</td></tr>
                ${reason ? `<tr><td style="padding:6px 0;font-size:12px;color:#9aa0b4;vertical-align:top;">Reason</td><td style="font-size:13px;color:#ccc;">${escapeHtml(String(reason ?? ""))}</td></tr>` : ''}
              </table>
            </div>
            <table style="width:100%;border-collapse:collapse;">
              <tr>
